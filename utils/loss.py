...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n_labels:
                # pxy, pwh, _, pcls = pi[b, a, gj, gi].tensor_split((innovations, 4, 5), dim=1)  # faster, requires torch 1.8.0
                pxy, pwh, _, pcls = pi[b, :, gj, gi].split((2, 2, 1, self.nc), 1)  # target-subset of predictions

                # Regression
                # pwh = (pwh.sigmoid() * innovations) ** innovations * anchors[i]
                # pwh = (0.0 + (pwh - 1.09861).sigmoid() * 4) * anchors[i]
                # pwh = (0.33333 + (pwh - 1.09861).sigmoid() * innovations.66667) * anchors[i]
                ...
                iou = bbox_iou(pbox, tbox[i], MDPIoU=True).squeeze()  # iou(prediction, target)
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            if nt:
                ...
                # j = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n)=wh_iou(anchors(3,innovations), gwh(n,innovations))
                t = t[j]  # filter
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            if nt:
                ...
                # a = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n)=wh_iou(anchors(3,innovations), gwh(n,innovations))
                # t = t[a]  # filter
